    ...
    btnDesconectar.addEventListener('click', async () => {
        ...
        try {
            ...
            if (dados.success) {
                ...
                alert(dados.message || "Erro ao desconectar.");
            }
        } catch (erro) {
...
async function carregarMensagensDoBanco() {
    try {
        ...
        if (msgNovo) msgNovo.value = dadosConfig.msg_novo || '';
        if (msgPreparo) msgPreparo.value = dadosConfig.msg_preparo || '';
        if (msgEntrega) msgEntrega.value = dadosConfig.msg_entrega || '';
        
        console.log('✅ Mensagens carregadas do banco!');
    } catch (erro) {
